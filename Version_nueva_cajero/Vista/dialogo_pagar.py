# BITCAFE
# VERSION 1.3 - QR Minimalista (Sin Referencia)
...
class DialogoPago(QDialog):
    def __init__(self, total_a_pagar_float, parent=None):
        ...
        # Ajustamos el tamaño a uno más compacto ya que quitamos la referencia
        self.setFixedSize(450, 500) 
        self.total_real = total_a_pagar_float
        self.metodo_seleccionado = "efectivo"

        # --- 1. CONFIGURACIÓN DEL BOTÓN DE ACCIÓN ---
        self.btn_finalizar = QPushButton("COBRAR")
        self.btn_finalizar.setFixedSize(180, 50) 
        ...
        self.input_cambio.setStyleSheet("QLineEdit { color: #D22A00; font-weight: 900; font-size: 20px; background-color: #FFF5F5; border: 1px solid #FFCCCC; }")
        l_efectivo.addWidget(self.input_cambio)
        l_efectivo.addStretch()
        self.tabs.addTab(tab_efectivo, "Efectivo")

        # --- TAB 2: TRANSFERENCIA (SOLO QR) ---
        tab_transf = QWidget()
        l_transf = QVBoxLayout(tab_transf)
        l_transf.setContentsMargins(20, 20, 20, 20)

        # Contenedor para el QR 
        self.lbl_qr = QLabel("Generando QR...") 
        self.lbl_qr.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_qr.setFixedSize(250, 250) # Un poco más grande para que luzca
        self.lbl_qr.setStyleSheet("""
            ...
            font-size: 14px;
        """)
        l_transf.addWidget(self.lbl_qr, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        btn_cancelar.setStyleSheet("""
            QPushButton { background-color: #FFFFFF; color: #555555; border: 2px solid #CCCCCC; border-radius: 10px; font-weight: bold; font-size: 14px; }
            QPushButton:hover { background-color: #F0F0F0; border: 2px solid #999999; color: #000000; }
        """)
    ...
    def mostrar_qr(self, pixmap):
        self.lbl_qr.setText("") 
        self.lbl_qr.setPixmap(pixmap.scaled(230, 230, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))

    def cambiar_metodo(self, index):
        if index == 0:
            self.metodo_seleccionado = "efectivo"
            self.btn_finalizar.setText("COBRAR")
            self.btn_finalizar.setStyleSheet("QPushButton { background-color: #D22A00; color: white; border-radius: 10px; font-weight: bold; font-size: 16px; border: none;} QPushButton:disabled { background-color: #FFCCCC; color: #E0E0E0; } QPushButton:hover { background-color: #B22400; }")
            self.calcular_cambio(self.input_pago.text())
        else:
            self.metodo_seleccionado = "transferencia"
            self.btn_finalizar.setText("PAGO CONFIRMADO")
            self.btn_finalizar.setStyleSheet("QPushButton { background-color: #009EE3; color: white; border-radius: 10px; font-weight: bold; font-size: 16px; border: none;} QPushButton:hover { background-color: #007EB5; }")
    ...
    def calcular_cambio(self, texto):
        ...
        try:
            ...
            if cambio >= -0.01:
                ...
                self.input_cambio.setText(f"Falta ${abs(cambio):.2f}")
                self.btn_finalizar.setEnabled(False)
        except ValueError:
            self.input_cambio.setText("---")
            self.btn_finalizar.setEnabled(False)
    ...
    def obtener_datos(self):
        ...
        # Retornamos referencia vacía siempre
        return self.metodo_seleccionado, monto, ""